...
class Host(explorerbase.ExplorerBase):
    ...
    def parsePrtdiag(self):
        ...
        for line in f:
            ...
            if 'OBP' in line:
                m=re.search('(?P<obp>OBP\s+\S+) %s\s+(?P<post>POST\s+\S+) %s' % (datestr, datestr), line)
                if m:
    ...
    def parseUname(self):
        ...
        if self.config['explorertype']=='solaris':
            ...
            if self.exists('sysconfig/uname-X.out'):
                ...
                for line in f:
                    line=line.rstrip()
                    if line.startswith('Release'): self['osrev']=line.split()[-1]
                    if line.startswith('KernelID'): self['kernelpatch']=line.split()[-1]
                    if line.startswith('Machine'): self['arch']=line.split()[-1]
                f.close()
