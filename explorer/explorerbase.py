...
class ExplorerBase(object):
    ...
    def glob(self, globexpr):
        globdir=os.path.join(self.config['datadir'], self.config['hostpath'], globexpr)
    ...
    def exists(self, filename):
        fullfilepath=os.path.join(self.config['datadir'], self.config['hostpath'], filename)
    ...
    def parseIostat(self, filename='disks/iostat_-E.out'):
        ...
        for line in f:
            ...
            if m:
                ...
                continue
            m=re.match("Vendor: (?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:(?P<serial>.*)",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("^(?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:(?P<serial>.*)",line)       # system info version
            if m:
                datadev[dev].update(m.groupdict())
                continue
            ...
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("^(?P<size>\S+) <(?P<bytes>-?\d+) bytes>",line)  # system info version
            if m:
                datadev[dev].update(m.groupdict())
                continue
            ...
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("^(?P<disk>c\d+t\d+d\d+)$", line)        # system info version
            if m:
                dev=m.group('disk')
                datadev[dev]={}
                datadev[dev].update(m.groupdict())
