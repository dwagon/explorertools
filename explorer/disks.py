...
class Disk(explorerbase.ExplorerBase):
    ...
    def cylinderMap(self):
        ...
        for bits in cylindermap:
            ...
            if first != oldend + 1:
                fullmap.append(
                    (oldend + 1, first - 1, first - oldend, "unalloc"))
            oldend = last
            lastcylinder = last
        ...
        if lastcylinder and diskcylinders and lastcylinder != diskcylinders:
            fullmap.append(
                (lastcylinder + 1, diskcylinders, diskcylinders - lastcylinder + 1, "unalloc"))
        return fullmap
    ...
    def analyse(self):
        ...
        if 'predfail' in self and self['predfail'] != '0':
            self.addIssue(
                'predfail', obj=self.name(), text='Disk %s predicted to fail' % self.name())
        elif 'harderrors' in self:
            # Small numbers of hard errors occur too often
            if int(self['harderrors']) > 50:
                self.addIssue('harderrors', obj=self.name(), text='Disk %s has %s hard errors' % (
                    self.name(), self['harderrors']))
            elif int(self['harderrors']) > 5:
                self.addConcern('harderrors', obj=self.name(
                ), text='Disk %s has %s hard errors' % (self.name(), self['harderrors']))
        ...
        if self.config['explorertype'] == 'solaris':
            ...
            if s2 not in self:
                if 'nobackupslice' in self and self['nobackupslice']:
                    ...
                    self.addConcern(
                        "nobackup", obj=self.name(), text='Disk %s has no backup slice' % self.name())
            ...
            if s2 in self and 'first_sector' in self[s2]:
                if self[s2]['first_sector'] != 0:
                    self.addConcern("backupslice", obj=self.name(
                    ), text='%s is malformed for a backup slice (%s-%s not whole disk)' % (s2, self[s2]['first_sector'], self[s2]['last_sector']))

        if self.unusedDisk():
            #self.Debug("%s=%s" % (self.name(), self.data))
            if 'nickname' in self:
                self.addConcern('unuseddisk', obj=self.name(
                ), text='Disk %s (%s) appears not to be used' % (self.name(), self['nickname']))
            else:
                self.addConcern(
                    'unuseddisk', obj=self.name(), text='Disk %s appears not to be used' % self.name())
    ...
    def __init__(self, config):
        ...
        for disk in self.st.diskList():
            ...
            for slice in self[disk]['slices']:
                self[disk][slice] = Slice(
                    config, slice, self.st[slice], self.st)
        self.analyse()
                ...
                if line.startswith('Disk:') and volume:
                    if volume not in self:
                        self[volume] = storage.Storage.initialDict(
                            {'_type': 'disk', 'description': 'Disk Drive', 'slices': set(), 'product': 'unknown', '_origin': filename})
        ...
        for line in f:
            ...
            if line.startswith('Path status') or line.startswith('State'):
                ...
                if status not in ('O.K.', 'ONLINE', 'STANDBY'):
                    self.addIssue(
                        'channel', obj=path, text="Channel path %s has bad status '%s'" % (path, status))
        f.close()
...
class storageDisks(explorerbase.ExplorerBase):
    ...
    def parseLinux_multipath_stanza(self, buffer):
        ...
        if name not in self:
            self[name] = storage.Storage.initialDict({'_type': 'mpath', 'description': 'Multipath', 'subdisks': set(
            ), '_origin': 'sos_commands/devicemapper/multipath_-v4_-ll'})
        subdisks = set()
        for line in buffer.splitlines():
            if line[0] == ' ':
                m = re.search(
                    '.*_ \d+:\d+:\d+:\d+\s+(?P<disk>\S+)\s+\d+:\d+\s+.*', line)
                if m:
        ...
        if class_ == 'HD':
            ...
            if device not in self:
                self[device] = storage.Storage.initialDict(
                    {'_type': 'disk', 'description': 'Disk Drive', 'slices': set(), '_origin': 'hardware.py', 'have_cylinders': False})
        ...
        for line in f:
            if line.startswith('*'):
                if line.find('bytes/sector') >= 0:
                    ...
                    self[disk]['accessible cylinders'] = int(
                        line.split()[1]) - 1
                ...
                if unalloc:
                    if len(line.split()) != 1:
                        self[disk]['unallocated_space'].append(
                            line[1:].strip())
                    else:
                        unalloc = False
            else:
                ...
                if self[disk]['have_cylinders']:
                    sl = self[slicename]
                    sl['first_cylinder'] = sl['first_sector'] / \
                        self[disk]['sectors/cylinder']
                    sl['last_cylinder'] = sl['last_sector'] / \
                        self[disk]['sectors/cylinder']
                    sl['cylinder_count'] = sl['sector_count'] / \
                        self[disk]['sectors/cylinder']
                try:
            ...
            if mode == 'controller':
                ...
                if len(bits) == 5:
                    ...
                    if disk in self:
                        ...
                        self.Warning(
                            "Disk %s found in raidctl not format" % disk)
                elif len(bits) == 4:
                    volume = bits[0]
                    disk = bits[2]
                    if disk in self:
    ...
    def postProcess(self, disk):
        ...
        if disk in self.channelmap:
            self[disk]['aliases'] = set(
                [d for d in self.channelmap[disk] if d != disk])

    ##########################################################################
    def isRemovable(self, disk):
        removables = ('CD-224E', 'SUN32XCD', 'DVD-ROM', 'ODD-DVD', 'DV-28E',
                      'CD-RW', 'CD-ROM', 'DV-28SL', 'FD-05PUB', 'CD/DVDW', 'DW-')
    ...
    def parseLuxInq(self, luxf):
        ...
        f.close()
        self.arrays[self.cmdfilename(luxf)] = {'path': path, 'product': product, 'vendor': vendor, 'ctrl': ctrl, 'name': self.cmdfilename(
            luxf).replace('luxadm_inq_dev_es_', '')}
    ...
    def parsePathToInst(self):
        ...
        for line in f:
            m = re.search(
                '"(?P<path>.*)" (?P<devnum>\S+) "(?P<devtype>.*)"', line)
            if m:
                self.devlist[
                    "%s%s" % (m.group('devtype'), m.group('devnum'))] = m.group('path')
        f.close()
            ...
            for arr in self.arrays:
                if path.startswith(self.arrays[arr]['ctrl']):
                    self[disk]['array'] = "%s %s %s" % (
                        self.arrays[arr]['name'], self.arrays[arr]['vendor'], self.arrays[arr]['product'])
    ...
    def alternativeParseFormat(self):
        ...
        for diskfile in disklist:
            ...
            self[disk] = storage.Storage.initialDict(
                {'_type': 'disk', 'description': 'Disk Drive', 'slices': set(), 'product': 'unknown', '_origin': diskfile})
        ...
        for line in f:
            ...
            for diskfmt in ['(?P<disk>[hs]d[a-z]{1,2})$', 'cciss/(?P<disk>c\dd\d)$', 'ida/(?P<disk>c\dd\d)$']:
                ...
                if m:
                    ...
                    break
            if diskname:
                self[diskname] = storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices': set(
                ), 'product': 'unknown', '_origin': 'proc/partitions', 'have_cylinders': False})
    ...
    def parseLinux_fdisk_chunk(self, lines):
        ...
        for line in lines:
            ...
            if line.startswith('Disk'):
                m = re.search(
                    'Disk (?P<device>/dev/.*): (?P<size>.*), (?P<bytes>\d+) bytes', line)
                if m:
                    disk = self.sanitiseDevice(m.group('device'))
                    if disk not in self:
                        self[disk] = storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices': set(
                        ), 'product': 'unknown', '_origin': 'fdisk_-l', 'have_cylinders': False})
                        self['disks'].add(disk)
                    self[disk]['size'] = m.group('size')
                    self[disk]['bytes'] = m.group('bytes')
                    continue
                n = re.search(
                    'Disk (?P<device>/dev/.*): (?P<heads>\d+) heads, (?P<sectors>\d+) sectors, (?P<cylinders>\d+) cylinders', line)
                if n:
            ...
            m = re.search(
                '(?P<heads>\d+) heads, (?P<sectors>\d+) sectors/track, (?P<cylinders>\d+) cylinders', line)
            if m:
            ...
            m = re.search(
                'Units = cylinders of (?P<facta>\d+) \* (?P<factb>\d+) = (?P<cylinder_size>\d+) bytes', line)
            if m:
                self[disk]['cylinder_size'] = int(m.group('cylinder_size'))
                continue

            m = re.search(
                'Units = cylinders of (?P<facta>\d+) \* (?P<factb>\d+)', line)
            if m:
                self[disk]['cylinder_size'] = int(
                    m.group('facta')) * int(m.group('factb'))
                continue

            m = re.search(
                '(?P<device>/dev/.*?)\s+(?P<boot>\*?)\s+(?P<start>\d+)\s+(?P<end>\d+)\s+(?P<blocks>\d+)\+?\s+(?P<id>..)\s+(?P<system>.*)', line)
            if m:
                ...
                if part not in self:
                    self.addConcern(
                        'unuseddisk', obj=self.name(), text='Disk %s potentially unused' % part)
                    continue
                ...
                self[part]['cylinder_count'] = self[part][
                    'last_cylinder'] - self[part]['first_cylinder']
        ...
        if bs in data and data[bs]['use']:
            for ns in list(data[disk]['slices']):
                ...
                if 'use' in self[ns] and self[ns]['use'] and self[ns]['slicenum'] != '8':
                    self.Warning("Overlap problem between %s %s and %s %s" % (
    ...
    def parseIostatE(self, filename='disks/iostat_-E.out'):
        ...
        for line in f:
            ...
            if m:
                ...
                continue
            m = re.match(
                "Vendor: (?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:\s*(?P<serial>.+?)", line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            ...
            # from elserwhere
            m = re.match(
                "Vendor: (?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:\s*", line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m = re.match(
    ...
    def describer(self, obj, data):
        if data[obj]['_type'] == 'slice':
            ...
            self.Fatal("describer passed object of type %s" %
                       data[obj]['_type'])
        ...
        for lev, partof in tmp:
            if partof in data:
                ...
                if 'description' in data[partof] and data[partof]['description']:
                    debugstr += '%s (description:)%s of ' % (partof,
        ...
        for po in data[disk]['partof']:
            ...
            if data[po]['_type'] == 'emcpower':
                ...
                if mounts:
                    ...
                    str += " No apparent use"
                    self.addConcern(
                        'unused emcpower', obj=po, text='EMC PowerPath %s has no obvious use' % po)
                break
