...
class Disk(explorerbase.ExplorerBase):
    ...
    def cylinderMap(self):
        ...
        for slice in self.sliceList():
            if 'first_cylinder' not in slice:
                continue
            if slice.isBackupSlice():
                continue
            slicenum=slice['slicenum']
            fs=int(slice['first_cylinder'])
            ls=int(slice['last_cylinder'])
            sc=int(slice['cylinder_count'])
    ...
    def analyse(self):
        ...
        if 'predfail' in self and self['predfail']!='0':
            self.addIssue('predfail', obj=self.name(), text='Disk %s predicted to fail' % self.name())
        elif 'harderrors' in self:
            if int(self['harderrors'])>50:      # Small numbers of hard errors occur too often
                self.addIssue('harderrors', obj=self.name(), text='Disk %s has %s hard errors' % (self.name(), self['harderrors']))
            elif int(self['harderrors'])>5:
                self.addConcern('harderrors', obj=self.name(), text='Disk %s has %s hard errors' % (self.name(), self['harderrors']))
        ...
        if self.config['explorertype']=='solaris':
            ...
            if s2 not in self:
                if 'nobackupslice' in self and self['nobackupslice']:
                    ...
                    pass        # Disks that have uses (e.g. entire disk is zfs, vxvm) generally don't need backup slices
                elif not self['slices']:
                    pass        # Disks that have no slices have much more serious problems
                else:
                    self.addConcern("nobackup", obj=self.name(), text='Disk %s has no backup slice' % self.name())

            # The only time this isn't true is when the disk/slice is used by vxvm but not in format/prtvtoc
            if s2 in self and 'first_sector' in self[s2]:
                if self[s2]['first_sector']!=0:
                    self.addConcern("backupslice", obj=self.name(), text='%s is malformed for a backup slice (%s-%s not whole disk)' % (s2, self[s2]['first_sector'], self[s2]['last_sector']))

        if self.unusedDisk():
            #self.Debug("%s=%s" % (self.name(), self.data))
            if 'nickname' in self:
                self.addConcern('unuseddisk', obj=self.name(), text='Disk %s (%s) appears not to be used' % (self.name(), self['nickname']))
            else:
                self.addConcern('unuseddisk', obj=self.name(), text='Disk %s appears not to be used' % self.name())
        ...
        for line in f:
            ...
            if mode=='controller':
                if line.startswith('Volume:'):
                    volume=line.split(':')[-1]
                if line.startswith('Disk:') and volume: # If no volume, not in use
                    if volume not in self:
                        self[volume]=storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices':set(), 'product':'unknown', '_origin': filename})
                ...
                if len(bits)==5:
                    volume=bits[0]
                    disk=bits[3]
                    badTag.append((self.raidCheck(line, 2, 'Raid Volume %s' % volume), volume))
                    badTag.append((self.raidCheck(line, 4, 'Raid Disk %s' % disk), disk))
                elif len(bits)==4:
                    volume=bits[0]
                    disk=bits[2]
                    badTag.append((self.raidCheck(line, 1, 'Raid Volume %s' % volume), volume))
                    badTag.append((self.raidCheck(line, 3, 'Raid Disk %s' % disk), disk))
                elif len(bits)==2:
                    disk=bits[1]
                    badTag.append((self.raidCheck(line, 1, 'Raid Disk %s' % disk), disk))
                else:
        ...
        for line in f:
            ...
            if line.startswith('Path status') or line.startswith('State'):
                ...
                if status not in ('O.K.', 'ONLINE', 'STANDBY'):
                    self.addIssue('channel', obj=path, text="Channel path %s has bad status '%s'" % (path, status))
        f.close()
...
class storageDisks(explorerbase.ExplorerBase):
    ...
    def parseLinux_pvs(self):
        ...
        for line in f:
            ...
            for i in (-1,-2):
                m=re.match('(?P<size>\d+\.\d\d)(?P<sizeunits>[GM])', bits[i])
                if m:
                    size=float(m.group('size'))
                    if m.group('sizeunits')=='M':
                        size*=1024
                    elif m.group('sizeunits')=='G':
                        size*=1024*1024
                    if device in self:
    ...
    def parseLinux_multipath_stanza(self, buffer):
        ...
        if name not in self:
            self[name]=storage.Storage.initialDict({'_type': 'mpath', 'description': 'Multipath', 'subdisks':set(), '_origin': 'sos_commands/devicemapper/multipath_-v4_-ll'})
        subdisks=set()
        for line in buffer.splitlines():
            if line[0]==' ':
                m=re.search('.*_ \d+:\d+:\d+:\d+\s+(?P<disk>\S+)\s+\d+:\d+\s+.*', line)
                if m:
    ...
    def parseLinux_hardwarepy_chunk(self, buff, class_):
        if class_=='HD':
            ...
            if device not in self:
                self[device]=storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices':set(), '_origin': 'hardware.py', 'have_cylinders': False})
        ...
        for line in f:
            if line.startswith('*'):
                ...
                if slicenum=='8':
                    ...
                    self['Boot Partition']=storage.Storage.initialDict({'_type': 'boot_partition' })
                if self[disk]['have_cylinders']:
                    sl=self[slicename]
                    sl['first_cylinder']=sl['first_sector']/self[disk]['sectors/cylinder']
                    sl['last_cylinder']=sl['last_sector']/self[disk]['sectors/cylinder']
                    sl['cylinder_count']=sl['sector_count']/self[disk]['sectors/cylinder']
                try:
    ...
    def postProcess(self, disk):
        ...
        """
        if ('product' not in self[disk] or self[disk]['product']=='unknown') and 'model' in self[disk]:
    ...
    def isRemovable(self, disk):
        removables=('CD-224E', 'SUN32XCD', 'DVD-ROM', 'ODD-DVD', 'DV-28E', 'CD-RW', 'CD-ROM', 'DV-28SL', 'FD-05PUB', 'CD/DVDW', 'DW-')
    ...
    def parseLuxInq(self, luxf):
        ...
        for line in f:
            ...
            if line.startswith('/devices'):
                path=line
                m=re.search('/devices(?P<ctrl>.*)/([^/]*)', path)
                ctrl=m.group('ctrl')
        ...
        f.close()
        self.arrays[self.cmdfilename(luxf)]={'path': path, 'product': product, 'vendor': vendor, 'ctrl':ctrl, 'name': self.cmdfilename(luxf).replace('luxadm_inq_dev_es_','')}
    ...
    def parsePathToInst(self):
        ...
        self.devlist={}
        for line in f:
            m=re.search('"(?P<path>.*)" (?P<devnum>\S+) "(?P<devtype>.*)"', line)
            if m:
                self.devlist["%s%s" % (m.group('devtype'), m.group('devnum'))]=m.group('path')
        f.close()
        ...
        for disk in self.diskList():
            ...
            for arr in self.arrays:
                if path.startswith(self.arrays[arr]['ctrl']):
                    self[disk]['array']="%s %s %s" % (self.arrays[arr]['name'],self.arrays[arr]['vendor'], self.arrays[arr]['product'])
    ...
    def alternativeParseFormat(self):
        ...
        for diskfile in disklist:
            disk=os.path.split(diskfile)[-1][:-2]       # Strip s2 off the end of the filename
            self[disk]=storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices':set(), 'product':'unknown', '_origin': diskfile})
    ...
    def parseLinux_partitions(self):
        ...
        for line in f:
            ...
            diskname=''
            for diskfmt in ['(?P<disk>[hs]d[a-z]{1,2})$', 'cciss/(?P<disk>c\dd\d)$', 'ida/(?P<disk>c\dd\d)$']:
                m=re.match(diskfmt,part)
                if m:
                    diskname=m.group('disk')
                    break
            if diskname:
                self[diskname]=storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices':set(), 'product': 'unknown', '_origin':'proc/partitions', 'have_cylinders': False})
                self['disks'].add(diskname)
                continue

            # Check to see if it matches known virtual disks
            isVirt=False
            for virtfmt in ['md\d', 'dm-\d', 'lvm.', 'loop\d']:
            ...
            matchob=None
            for partfmt in [
                    '(?P<slicehandle>(?P<disk>[hs]d[a-z]{1,2})(?P<slice>\d))',
                    'cciss/(?P<slicehandle>(?P<disk>c\dd\d)(?P<slice>p\d))',
                    '(?P<slicehandle>(?P<disk>emcpower)(?P<slice>\S))',
                    'ida/(?P<slicehandle>(?P<disk>c\dd\d)(?P<slice>p\d))',
                    ]:
    ...
    def parseLinux_fdisk_chunk(self, lines):
        ...
        for line in lines:
            ...
            if 'emcpower' in line:
                return
            if self.lineSkipper(line, start=['<---', 'Device'], middle=['DOS and BSD magic', 'BSD mode', 'does not end on']):
                continue
            if line.startswith('Disk'):
                m=re.search('Disk (?P<device>/dev/.*): (?P<size>.*), (?P<bytes>\d+) bytes', line)
                if m:
                    disk=self.sanitiseDevice(m.group('device'))
                    if disk not in self:
                        self[disk]=storage.Storage.initialDict({'_type': 'disk', 'description': 'Disk Drive', 'slices':set(), 'product': 'unknown', '_origin':'fdisk_-l', 'have_cylinders': False})
                        self['disks'].add(disk)
                    self[disk]['size']=m.group('size')
                    self[disk]['bytes']=m.group('bytes')
                    continue
                n=re.search('Disk (?P<device>/dev/.*): (?P<heads>\d+) heads, (?P<sectors>\d+) sectors, (?P<cylinders>\d+) cylinders', line)
                if n:
            ...
            m=re.search('(?P<heads>\d+) heads, (?P<sectors>\d+) sectors/track, (?P<cylinders>\d+) cylinders', line)
            if m:
            ...
            m=re.search('Units = cylinders of (?P<facta>\d+) \* (?P<factb>\d+) = (?P<cylinder_size>\d+) bytes', line)
            if m:
                self[disk]['cylinder_size']=int(m.group('cylinder_size'))
                continue

            m=re.search('Units = cylinders of (?P<facta>\d+) \* (?P<factb>\d+)', line)
            if m:
                self[disk]['cylinder_size']=int(m.group('facta'))*int(m.group('factb'))
                continue

            m=re.search('(?P<device>/dev/.*?)\s+(?P<boot>\*?)\s+(?P<start>\d+)\s+(?P<end>\d+)\s+(?P<blocks>\d+)\+?\s+(?P<id>..)\s+(?P<system>.*)', line)
            if m:
                part=self.sanitiseDevice(m.group('device'))

                # Don't know why this happens (not in /proc/partitions) - disk not used?
                if part not in self:
                    self.addConcern('unuseddisk', obj=self.name(), text='Disk %s potentially unused' % part)
                    continue

                if m.group('system') in ('Extended', "Win95 Ext'd (LBA)"):      # Not a real partition
                    del self[part]
                ...
                self[part]['cylinder_count']=self[part]['last_cylinder']-self[part]['first_cylinder']
    ...
    def parseSolaris_format(self):
        ...
        for line in f:
            line=line.strip()
            m=re.search('\d+. (?P<disk>\S+) <(?P<details>.*)>', line)
            if m:
                disk=m.group('disk')
                if 'emcpower' in disk:
                    continue
                self[disk]=storage.Storage.initialDict({
                    '_type': 'disk', 'description': 'Disk Drive', 'slices':set(),
                    'product': 'unknown', '_origin': 'disks/format.out',
                    })
    ...
    def crossPopulateDisk(self, data, disk):
        ...
        if bs in data and data[bs]['use']:
            for ns in list(data[disk]['slices']):
                ...
                if ns==bs:
                    continue
                if 'use' in self[ns] and self[ns]['use'] and self[ns]['slicenum']!='8':
                    self.Warning("Overlap problem between %s %s and %s %s" % (ns, list(data[ns]['use']), bs, list(data[bs]['use'])))
                else:
                    if ns in data:
                        #self.Debug("Removing slice %s as it overlaps with a used backup slice" % ns)
                        # If we are the boot partition - we can be used and not used at the same time
                        if data[ns]['slicenum']=='8' and ns in data['Boot Partition']['contains']:
    ...
    def crossPopulateProtection(self, disk, data):
        # If a disk slice has protection then the use/filesystem on it also has protection
        for slice in data[disk]['slices']:
            ...
            for use in data[slice]['use']:
                if protect:
                    if use in data:
                        ...
                        # Quorums aren't part of the whole 'storage family' but they still use a disk
                        pass
                    else:
    ...
    def calculateUsed(self, data):
        ...
        for disk in self.diskList():
            ...
            used=False
            for slice in data[disk]['slices']:
                if data[slice]['slicenum']=='8':        # Boot Partition always exists
                    continue
                if data[slice]['use']:
                    used=True
                    break
    ...
    def parseIostatE(self, filename='disks/iostat_-E.out'):
        ...
        for line in f:
            ...
            if not line:
                continue
            m=re.match("(?P<device>\S+)\s*Soft Errors: (?P<softerrors>\d+) Hard Errors: (?P<harderrors>\d+) Transport Errors: (?P<transerrors>\d+)",line)
            if m:
                dev=m.group('device')
                datadev[dev]={}
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("Vendor: (?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:\s*(?P<serial>.+?)",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            # Same as above but with no serial to prevent overwriting a serial from elserwhere
            m=re.match("Vendor: (?P<vendor>.*?)\s+Product: (?P<product>.*?)\s+Revision: (?P<revision>.*) Serial No:\s*",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("Model: (?P<model>.*) Revision: (?P<revision>.*) Serial No: (?P<serial>.*)",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            ...
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("Media Error: (?P<mediaerror>\d+) Device Not Ready: (?P<devnotready>\d+)\s+No Device: (?P<nodev>\d+) Recoverable: (?P<recoverable>\d+)",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("Illegal Request: (?P<illreq>\d+) Predictive Failure Analysis: (?P<predfail>\d+)",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
            ...
            if m:
                datadev[dev].update(m.groupdict())
                continue
            m=re.match("RPM: (?P<rpm>\d+) Heads: (?P<heads>\d+) Size: (?P<size>\S+) <(?P<bytes>-?\d+) bytes>",line)
            if m:
                datadev[dev].update(m.groupdict())
                continue
        f.close()
        ...
        for lev,partof in tmp:
            if partof in data:
                ...
                if data[partof]['_type']=='boot_partition':
                    break
                if data[partof]['_type']=='filesystem': # This should be 'use' so dont report it
                    continue
                if data[partof]['_type']=='did_slice':  # This should be 'use' so dont report it
                    continue

                # Default describer
                debugstr+="&lt;%s-%s&gt;" % (partof,data[partof]['_type'])
                if 'description' in data[partof] and data[partof]['description']:
                    debugstr+='%s (description:)%s of ' % (partof, data[partof]['description'])
                    str+='%s %s of ' % (partof, data[partof]['description'])
                else:
                    str+='%s (?)' % partof
            else:
                str+='(unknown %s)' % partof
        if softparcount:        # You can get hundreds of these which makes it unreadable
            str+=" %d soft partitions" % softparcount
        if str.endswith(' of '):        # Trim last ' of ' off
            str=str[:-4]
        debugstr+='%s' % tmp
        return str
        #return "%s <font color=blue>[%s]</font>" % (str, debugstr)     # Debugging
        ...
        for po in data[disk]['partof']:
            ...
            if data[po]['_type']=='emcpower':
                ...
                if mounts:
                    ...
                    str+=" No apparent use"
                    self.addConcern('unused emcpower', obj=po, text='EMC PowerPath %s has no obvious use' % po)
                break
