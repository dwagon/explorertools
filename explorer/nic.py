...
class Nic(explorerbase.ExplorerBase):
    ...
    def post(self):
        ...
        for iface in self['interfaces']:
            self['interfaces'][iface]['cidr'] = self.getCIDR(
                self['interfaces'][iface])
            self['interfaces'][iface]['network'] = self.getNetwork(
                self['interfaces'][iface])
    ...
    def analyse(self):
        ...
        if 'half' in self['link_duplex'] and 'normal' not in self['link_duplex']:
            self.addConcern(
                "duplex", obj=self.objname, text="set to half-duplex")
        for iface in self['interfaces']:
            if 'flags' in self['interfaces'][iface]:
                if 'UP' not in self['interfaces'][iface]['flags']:
                    if 'IPv6' in self['interfaces'][iface]['flags'] and iface == 'lo0':
                        ...
                        self.addConcern(
                            "down", obj=self.objname, text="interface down")
    ...
    def parseNdd(self):
        ...
        """
        nic = re.sub(
            '(?P<nicname>\D+)(?P<inst>\d+)', '\g<nicname>.\g<inst>', self.name())
    ...
    def getLinkStatus(self):
        if 'ndd_link_status' in self:
            self['link_status'] = {'0': 'down', '1': 'up'}[
                self['ndd_link_status']]
    ...
    def getLinkSpeed(self):
        if 'ndd_link_speed' in self:
            if self['ndd_link_speed'] == '1000':
                ...
                self.Warning("Unhandled ndd_link_speed=%s" %
                             self['ndd_link_speed'])
    ...
    def getLinkDuplex(self):
        if 'ndd_link_mode' in self:
            if self['ndd_link_mode'] == '0':
                ...
                self.Warning("Unhandled ndd_link_mode=%s" %
                             self['ndd_link_mode'])
        ...
        if self['dev'] in ('bge', 'ixge', 'nge'):
            ...
            self['link_duplex'] = {
                '-1': 'error', '0': 'unknown', '1': 'half', '2': 'full'}[self.getKstatDuplex()]
        elif self['dev'] == 'dmfe':
            ...
            if 'ndd_link_duplex' in self:
                self['link_duplex'] = {
                    '-1': 'unknown', '0': 'half', '1': 'half or full', '2': 'full'}[self['ndd_link_duplex']]
            else:
                self.Warning(
                    "Unhandled nic: %s in getLinkDuplex()" % self['dev'])
    ...
    def getNetwork(self, dct):
        ...
        try:
            ...
            nm = int(quads[3]) + 256 * (int(quads[2]) + 256 *
                                        (int(quads[1]) + 256 * int(quads[0])))
        order = 3
        ip = 0
...
class Nics(explorerbase.ExplorerBase):
    ...
    def parseKstats(self):
        if self.config['explorertype'] != 'solaris':
            return
        virtchains = ['fcip', 'ipdrop', 'chipinfo', 'mii', 'statistics', 'parameters', 'phydata', 'driverinfo', 'zero_copy', 'dmfe_events', 'chipid', 'inbound', 'outbound', 'tcpstat_g',
                      'mac', 'serdes', 'FFLP Stats', 'IPP Stats', 'MMAC Stats', 'Port Stats', 'RDC Channel', 'RDC System Stats', 'TDC Channel', 'TXC Stats', 'ZCP Stats', 'driver-debug', 'vsw']
        ...
        for link in k.classChains('net'):
            ...
            if not found:
                self[nicname] = Nic(
                    self.config, nicname, link.module, link.instance)
    ...
    def parseLinux_ifconfig_interface(self, data):
        ...
        if nicname not in self:
            self[nicname] = Nic(
                self.config, nicname, m.group('dev'), m.group('inst'))
        ...
        for line in data:
            m = re.search(
                'inet addr:(?P<ipaddr>.*?)\s+Bcast:(?P<broadcast>.*?)\s+Mask:(?P<mask>.*)', line)
            if m:
            ...
            if m:
                ...
                continue
            m = re.search(
                'inet addr:(?P<ipaddr>.*?)\s+Mask:(?P<mask>.*)', line)
            if m:
                nicobj['ipaddr'] = m.group('ipaddr')
                nicobj['netmask'] = m.group('mask')
                continue
            m = re.search(
                '.*Link encap:(?P<linkencap>.*?)\s+HWaddr (?P<hwaddr>.*)', line)
            if m:
            ...
            if m:
                ...
                continue
            m = re.search(
                '(?P<flags>.*)MTU:(?P<mtu>\d+)\s+Metric:(?P<metric>\d+)', line)
            if m:
    ...
    def parseSolaris_ifconfig(self):
        ...
        for line in f:
            ...
            if line[0] == '\t':
                m = re.search(
                    'inet (?P<ipaddr>.*) netmask (?P<netmask>.*) broadcast (?P<broadcast>.*)', line)
                if m:
                    nicobj['ipaddr'] = m.group('ipaddr')
                ...
                m = re.search(
                    '(?P<ifname>.*): flags=(?P<flagbits>.*)<(?P<flags>.*)> mtu (?P<mtu>\d+)( index (?P<index>\d+))?', line)
                if not m:
