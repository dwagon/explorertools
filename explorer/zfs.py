...
class ZfsPool(explorerbase.ExplorerBase):
    ...
    def analyse(self):
        if self['health'] != "ONLINE":
            self.addIssue("poolhealth", obj=self.objname,
            ...
            if subpool == pool:
                ...
                if subpool not in self:
                    self[subpool] = storage.Storage.initialDict(
                        {'_type': 'zfs_subpool', 'description': 'ZFS SubPool', 'contains': set([pool]), '_origin': filename})
    ...
    def crossPopulatePool(self, pool, data):
        ...
        for mp in self[pool]['use']:
            if mp in data:
                ...
                self.addConcern(
                    "mount", obj=mp, text="ZFS pool %s has filesystem %s which isn't mounted" % (pool, mp))
