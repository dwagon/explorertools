...
explorereg=[
    ("explorer.(?P<hostid>[a-f0-9]{8}).(?P<hostname>.*)-(?P<timedate>(?P<date>\d{4}.\d\d.\d\d).(?P<time>\d\d.\d\d))", "solaris"),
...
def readConfig(cfg=None):
    ...
    options={}
    defaults={
        'agedcare': True, # agedcare: Send old files to an old directory rather than delete
        'bindir': '/app/explorer/bin', # bindir: Where the explorer tools executables are kept
        'changelog': False, # changelog: Write a log of the changes generated for this host
        'changelogdir': '/app/explorer/changelog', # changelogdir: Where to put the changelogs
        'datadir': '/app/explorer/data', # datadir: Where to look for the data files
        'hostinfo': True, # hostinfo: Generate hostinfo updates
        'hostinfodir': '/app/hostinfo/bin', # Where hostinfo commands are installed
        'oldage': 365, # oldage: how many days old an explorer is before it is ignored
        'outputdir': '/app/explorer/output', # outputdir: Where to create the HTML outputs
        'retain_compressed': 1, # retain_compressed: How many compressed files to keep
...
def allExplorers(reg=''):
    ...
    files=[f.replace('%s/' % options['datadir'], '') for f in glob.glob(globstr) if os.path.splitext(f)[1] not in ('.gz', '.bz2', '.md5')]
    for fn in files:
        ...
        try:
            d=m.group('date')
        except IndexError:      # sosreport on linux doesn't have the date encoded :(
            d=time.strftime("%Y.%m.%d.%H.%M.%s", time.localtime(os.path.getmtime(fullfile)))
