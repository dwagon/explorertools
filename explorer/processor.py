...
class Processors(explorerbase.ExplorerBase):
    ...
    def parse(self):
        try:
            if self.config['explorertype']=='solaris':
                ...
                self.Fatal("Processors - unknown explorertype %s" % self.config['explorertype'])
    ...
    def parseLinux_cpuinfo(self):
        ...
        for line in f:
            ...
            if line.startswith('cpu MHz'):
                # Convert speed to a round number: 1500 rather than 1499.998 MHz
                cpu['speed']="%s" % int(math.ceil(float(line[line.find(':')+1:].strip())))
        f.close()
    ...
    def parseSolaris_psrinfo(self):
        ...
        for line in f:
            ...
            cpu=Processor(self.config, cpunum)
            m=re.search('The (?P<proctype>.*) processor operates at (?P<speed>.*),', line)
            if m:
