...
class VxvmVolume(explorerbase.ExplorerBase):
    ...
    def analyse(self):
        if self['use'] == 'Unused':
            self.addConcern(
                'volume', obj=self.name(), text="Volume is not used")
...
class VxvmDiskgroup(explorerbase.ExplorerBase):
    ...
    def analyse(self):
        if 'vxvm_status' in self and self['vxvm_status'] == 'error':
            self.addIssue(
                'diskgroup', obj=self.name(), text="Status is 'error'")
            ...
            for dev in self[diskgroup]['devices']:
                if dev not in data:
                    ...
                    self[dev] = storage.Storage.initialDict(
                        {'_type': 'missing', 'missedby': diskgroup, 'missed_at': 'crossPopulate'})
                else:
        ...
        for line in f:
            ...
            if diskgroup.startswith('('):
                ...
                if diskgroup not in self:
                    self[diskgroup] = storage.Storage.initialDict({'_type': 'vxvm_diskgroup', 'volumes': set(
            ...
            if device not in self:
                ...
                self[device] = storage.Storage.initialDict(
                    {'_type': 'missing', 'missedby': diskgroup, 'missed_at': 'parseVxDisk'})
            self[device].update(
                {'diskgroup': diskgroup, 'vxvm_status': status})
        f.close()
