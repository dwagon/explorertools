...
class Metadev(explorerbase.ExplorerBase):
    ...
    def analyse(self):
        ...
        if 'use' not in self or not self['use']:
            self.addConcern("useless metadev", obj=self.name(), text="No use found for %s %s (devices %s)" % (
    ...
    def mirrorCheck(self):
        ...
        if 'contains' in self:
            for tmp in self['contains']:
                try:
                    ...
                    self.Warning(
                        "Couldn't find %s in data as in %s's contains" % (tmp, self.name()))
        else:
            if 'use' in data:
        ...
        for mount in mounts:
            ...
            if numsubs < 2 and not self['protected']:
                self.addIssue(
                    'onesidedmirror', obj=mount, text="%s (%s) - one sided mirror" % (mount, self.name()))
            elif numsubs > 2:
                self.addConcern(
                    'multiwaymirror', obj=mount, text="%s (%s) - %d way mirror" % (mount, self.name(), numsubs))
...
class Volmanager(explorerbase.ExplorerBase):
    ...
    def analyseMetadb(self):
        ...
        if len(self['metadb']['devices']) < 2:
            self.addIssue(
                "metadb diversity", obj='metadb', text="Insufficient diversity of metadbs")
        if len(self['metadb']['copies']) < 2:     # Should be 3
            self.addIssue("too few metadb", obj='metadb', text="Insufficient number of metadbs (%d copies)" % (
        ...
        for metadb in self['metadb']['copies']:
            if set(self[metadb]['flags']).intersection(string.uppercase):
                self.addIssue("failed metadb", obj=list(self[metadb]['contains'])[
                              0], text="Failure on metadb: %s" % self[metadb]['flags'])
        ...
        for dev, count in devs.items():
            ...
            if otheruses:
                self.addConcern(
                    "mounted metadb", obj=dev, text="Slice used for more than metadb: %s" % ",".join(list(otheruses)))

            if numdbs - count < 2:
                self.addIssue("too few metadb on failure", obj=dev,
                              text="Insufficient local metadbs if %s failed (%d left)" % (dev, (numdbs - count)))
                continue
            pct = (numdbs - count) / float(numdbs)
            if pct < 0.5:
                self.addIssue("too low percentage metadb on failure", obj=dev,
                              text="Insufficient local metadbs if %s failed (%d%% left)" % (dev, int(100 * pct)))
    ...
    def analyse(self):
        ...
        for md in self.metaList():
            if md not in self:
                self.Warning(
                    "md %s doesn't exist in data but in metaList" % md)
                continue
            self[md].analyse()
        ...
        for line in buff:
            if line.startswith('md'):
                ...
                for devbit in bits[4:]:
                    if '(F)' in devbit:
                        self.addIssue(
                            'failed device', obj=metadev, text="Failed metadev: %s" % line)
...
class storageVolmanager(explorerbase.ExplorerBase):
    ...
    def crossPopulateMetadbs(self, data):
        ...
        for dev, count in devcounts.items():
            if dev not in data:
                self.Warning(
                    "Device %s has a metadb on it but doesn't exist" % dev)
                continue
        ...
        for k in self.keys():
            ...
            for slice in slicelist:
                ...
                data[slice] = storage.Storage.initialDict(
                    {'_type': 'missing', 'missedby': k, 'missed_at': 'crossPopulate - 1'})
            ...
            if obj['_type'] in ('metadb_copy', 'disksuite'):
                for dev in obj['devices']:
                    if dev in data:
                        ...
                        self.Warning(
                            "Diskset %s %s lives on unknown disk %s" % (obj['diskset'], obj['_type'], dev))
                        data[dev] = storage.Storage.initialDict({'_type': 'missing', 'missedby': 'diskset %s %s %s' % (
                            obj['diskset'], obj['_type'], objname), 'missed_at': 'crossPopulateDiskset'})
        ...
        for line in didadm:
            ...
            if name not in self:
                self["did/%s" % name] = storage.Storage.initialDict(
                    {'_type': 'did_disk', '_origin': filename, 'description': "did disk"})
        ...
        for did in self['_didmap']:
            for slice in [0, 2]:
                ...
                self[didslice] = storage.Storage.initialDict(
                    {'_type': 'did_slice', '_origin': filename, 'description': "did slice"})
        ...
        for line in f:
            ...
            if lv.startswith('['):
                ...
                if vl in volabels:
                    ...
                    self[vl] = storage.Storage.initialDict(
                        {'_type': 'logvol', 'description': 'LVM Volume Label', '_origin': lvs_file})
                else:
                    ...
                    if attrs.startswith('l'):   # Logs
                        for ln in buff:
                            if vl in ln:
                                ...
                                break
                        self[vl] = storage.Storage.initialDict(
                            {'_type': 'logvol', 'description': 'LVM Mirror Log Volume', '_origin': lvs_file})
                self[vl]['partof'].add(vg)
            elif lvlabel not in self:
                self[lvlabel] = storage.Storage.initialDict(
                    {'_type': 'logvol', 'description': 'LVM Logical Volume', '_origin': lvs_file})
            ...
            if vglabel not in self:
                self[vglabel] = storage.Storage.initialDict(
                    {'_type': 'volgroup', 'description': 'LVM Volume Group', '_origin': lvs_file})
    ...
    def parse(self):
        ...
        if self.config['explorertype'] == 'solaris':
            ...
            for metadev in self.metaList():
                if metadev not in self:
                    self.Warning(
                        "Couldn't find metadev %s even though it is in metaList" % metadev)
                    continue
        ...
        if 'diskset_copies' not in foreign['metadb']:
            ...
            for metadb in foreign['metadb']['diskset_copies']:
                if 'did_dev' not in foreign[metadb]:
                    self.Warning("Couldn't find the did_dev in foreign metadb %s from host %s" % (
                        metadb, proxyhost))
                    break
                ...
                if diddev in foreign['_proxy_didmap']:
                    for dd in foreign['_proxy_didmap'][diddev]:
                        ...
                        copyname = self.addMetadb(localdevice, {'flags': foreign[metadb][
                                                  'flags'], 'diskset': diskset, 'foreign': proxyhost, 'did_dev': dd})
    ...
    def parseMetaDb(self, filename):
        ...
        for line in f:
            ...
            if 'did' in bits[-1]:
                devlist = [self.sanitiseDevice(d)
                           for d in self.didLookup(bits[-1])]
            else:
                devlist = [self.sanitiseDevice(bits[-1])]

            for device in devlist:
                copyname = self.addMetadb(device, {
                                          'first_block': bits[-3], 'blockcount': bits[-2], 'flags': "".join(bits[:-3])})
        ...
        for obj in foreign['_diskset_%s' % diskset]:
            for did in foreign[obj]['did_dev']:
                ...
                if diddisk not in foreign['_proxy_didmap']:
                    self.Warning("Couldn't find %s in %s' didmap" %
                                 (diddisk, proxyhost))
                    continue
    ...
    def parseConcat(self, metadev, bits, diskset):
        ...
        if bits[0] == '1' and bits[1] == '1':
            ...
            if bits[0] == '1':
                ...
                metadev[
                    'description'] = '%s-Way Concat of %s-Way Stripe' % (bits[1], bits[0])
        metadev['did_dev'] = set()
