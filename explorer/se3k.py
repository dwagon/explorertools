            ...
            try:
                ...
                if mode == 'fru':
                    ...
                    if line.startswith('FRU Status:'):
                        ...
                        if status != 'OK':
                            self.addIssue("SE3k_FRU", obj="%s %s" % (
                                self.name(), fru), text="FRU (%s) %s has status %s" % (self.name(), fru, status))
                if mode == 'status':
                    if line.startswith('----') or line.startswith('Id'):
                        continue
                    if line.split()[-1].strip() != 'OK':
                        self.addIssue("SE3k_Enclosure", obj="%s" % self.name(
                        ), text="Enclosure (%s) has status %s" % (self.name(), line.split()[-1]))
                if mode == 'component':
                    ...
                    if status not in ('OK', 'Unknown'):
                        self.addIssue("SE3k_Component", obj="%s %s" % (
                            self.name(), component), text="Component %s has status %s" % (component, status))
            except:
    ...
    def parseShowLogicalDrives(self):
        ...
        for line in f:
            ...
            if line.startswith('ld'):
                ...
                if bits[8] != 'Good':
                    self.addIssue("logical", obj="%s %s" % (self.name(), bits[
                                  0]), text="Logical Drive (%s) %s has status %s" % (self['controller'], bits[0], bits[8]))
        f.close()
    ...
    def parseShowEnclosureStatus(self):
        ...
        for line in f:
            ...
            if bits[2] not in ('OK', 'Unknown'):
                self.addIssue("component", obj="%s %s" %
                              (bits[0], bits[1]), text='%s\n%s' % (header, line))
        f.close()

    ##########################################################################
    def getDetails(self, line):
        m = re.search(
            'sccli: selected device /dev/(rdsk|es)/(?P<device>\S+) \[(?P<hardware>.*) (?P<serial>SN#.*)\]', line)
        if m:
    ...
    def parseShowDisks(self):
        ...
        for line in f:
            ...
            if status not in ('ONLINE', 'STAND-BY'):
                self.addIssue("disk", obj=id, text='Array %s-%s disk %s has status %s' %
